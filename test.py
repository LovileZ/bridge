...
def deploy_contract(w3, contract_file, admin_address):
    ...
    balance = w3.eth.get_balance(warden_address)
    estimated_cost = 3000000 * w3.eth.gas_price
    
    if balance < estimated_cost:
        print("\nInsufficient funds for deployment!")
        print(f"Current balance: {w3.from_wei(balance, 'ether')} ETH")
        print(f"Estimated cost: {w3.from_wei(estimated_cost, 'ether')} ETH")
        print(f"Missing: {w3.from_wei(estimated_cost - balance, 'ether')} ETH")
        
        if 'destination' in contract_file:
            print("\nTo get BSC testnet BNB, visit:")
            print("https://testnet.bnbchain.org/faucet-smart")
        else:
            print("\nTo get AVAX testnet tokens, visit:")
            print("https://faucet.avax.network/")
            
        return None, None, None, None

    # Modify transaction parameters to use less gas
    construct_txn = contract.constructor(admin_address).build_transaction({
        'from': warden_address,
        'nonce': nonce,
        'gas': 1500000,  # Reduced from 3000000
        'maxFeePerGas': w3.eth.gas_price,
        'maxPriorityFeePerGas': int(w3.eth.gas_price * 0.8)  # Lower priority fee
    })
...
def deploy_bridge_contracts():
    ...
    grant_tx = source_contract.functions.grantRole(
        ...
        'gas': 200000,  # Increased from 200
        'maxFeePerGas': source_w3.eth.gas_price,
        'maxPriorityFeePerGas': source_w3.eth.gas_price
    })
    ...
    grant_tx = dest_contract.functions.grantRole(
        ...
        'gas': 200000,  # Increased from 200
        'maxFeePerGas': dest_w3.eth.gas_price,
        'maxPriorityFeePerGas': dest_w3.eth.gas_price
    })
...
def register_tokens():
    ...
    for _, token in source_tokens.iterrows():
        ...
        tx = destination_contract.functions.createToken(
            ...
            'gas': 3000000,  # Keep this value as is
            'maxFeePerGas': dest_w3.eth.gas_price,
            'maxPriorityFeePerGas': dest_w3.eth.gas_price
        })
